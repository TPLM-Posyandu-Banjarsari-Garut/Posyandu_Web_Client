...
import { useForgetPasswordBidan } from "@/hooks/query/authBidan/useForgetPasswordBidan";

interface ForgetPasswordForm {
  email: string;
}

export default function LupaPasswordBidan() {
  const router = useRouter();
  const { register, handleSubmit, formState: { errors } } = useForm<ForgetPasswordForm>();
  const forgetPasswordMutation = useForgetPasswordBidan();

  const onSubmit = (data: ForgetPasswordForm) => {
    forgetPasswordMutation.mutate(data.email, {
      onSuccess: () => {
        router.push(`/bidan/lupa-password/otp?email=${encodeURIComponent(data.email)}`);
      },
    });
  };
