...
import { useForgetPasswordKader } from "@/hooks/query/authKader/useForgetPasswordKader";

interface ForgetPasswordForm {
  email: string;
}

export default function LupaPasswordKader() {
  const router = useRouter();
  const { register, handleSubmit, formState: { errors } } = useForm<ForgetPasswordForm>();
  const forgetPasswordMutation = useForgetPasswordKader();

  const onSubmit = (data: ForgetPasswordForm) => {
    forgetPasswordMutation.mutate(data.email, {
      onSuccess: () => {
        router.push(`/kader/lupa-password/otp?email=${encodeURIComponent(data.email)}`);
      },
    });
  };
