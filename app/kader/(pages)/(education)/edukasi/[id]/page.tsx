        ...
        return (
            <div className="min-h-screen bg-slate-100 font-sans pb-10 pt-4 px-2 sm:px-0 text-slate-800 flex justify-center">
                <div className="w-full max-w-md bg-white min-h-[90vh] rounded-[2.5rem] relative shadow-2xl overflow-hidden flex flex-col border-[6px] border-white ring-1 ring-slate-200 justify-center items-center gap-4">
                    ...
                    <Link href="/kader/edukasi" className="mt-4 px-6 py-3 bg-blue-600 text-white rounded-[1.25rem] text-sm font-bold shadow-[0_4px_15px_rgba(37,99,235,0.3)] hover:bg-blue-700 active:scale-95 transition-all">Kembali ke Daftar</Link>
                </div>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-slate-100 font-sans pb-10 pt-4 px-2 sm:px-0 text-slate-800 flex justify-center">
            <div className="w-full max-w-md bg-white min-h-[90vh] rounded-[2.5rem] relative shadow-2xl overflow-hidden flex flex-col border-[6px] border-white ring-1 ring-slate-200">
                ...
                <div className="relative z-10 px-6 pt-8 flex items-center justify-between">
                    <Link href="/kader/edukasi" className="p-2 -ml-2 rounded-full bg-white/20 text-white backdrop-blur-md hover:bg-white/30 transition-colors">
                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15 19l-7-7 7-7" />
                        </svg>
                    </Link>
                    <h1 className="text-lg font-bold text-white tracking-wide">Detail Edukasi</h1>
                    <Link href={`/kader/edit-edukasi/${education.id}`} className="px-4 py-1.5 bg-white text-indigo-600 rounded-full text-xs font-bold hover:bg-blue-50 shadow-sm transition-colors">
                        Edit
                    </Link>
                </div>
