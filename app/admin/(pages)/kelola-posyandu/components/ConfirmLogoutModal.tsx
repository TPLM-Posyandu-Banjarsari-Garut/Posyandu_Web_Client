  ...
  return (
    <div className="fixed inset-y-0 left-1/2 -translate-x-1/2 w-full max-w-md bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-6 animate-fade-in">
      <div className="bg-white w-full max-w-[280px] rounded-[2rem] p-6 shadow-2xl border border-slate-100 flex flex-col items-center justify-center animate-scale-up text-center">
        <div className="w-14 h-14 rounded-full bg-rose-50 text-rose-505 flex items-center justify-center mb-4 shadow-[0_4px_12px_rgba(244,63,94,0.15)]">
          <svg
            className="w-7 h-7"
            fill="none"
            stroke="currentColor"
        ...
        <div className="flex w-full gap-2.5">
          <button
            onClick={onClose}
            className="flex-1 bg-slate-50 border border-slate-200 text-slate-500 font-bold text-xs py-3 rounded-xl hover:bg-slate-100 transition-colors cursor-pointer"
          >
            Batal
          </button>
          <button
            onClick={onConfirm}
            disabled={isPending}
            className="flex-1 bg-rose-600 text-white font-bold text-xs py-3 rounded-xl hover:bg-rose-700 active:scale-95 transition-all shadow-md shadow-rose-600/10 disabled:opacity-50 cursor-pointer"
          >
