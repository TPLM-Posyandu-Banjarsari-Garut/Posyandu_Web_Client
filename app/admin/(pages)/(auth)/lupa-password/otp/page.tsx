...
import { useResetPasswordAdmin } from "@/hooks/query/authAdmin/useResetPasswordAdmin";
import { ResetPasswordOTPPayload } from "@/interfaces/auth";

export default function OTPResetPasswordAdmin() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-slate-100"><div className="w-8 h-8 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div></div>}>
      <OTPResetPasswordAdminContent />
    </Suspense>
  );
}

function OTPResetPasswordAdminContent() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-100">
        <div className="bg-white p-8 rounded-2xl shadow-sm text-center">
          ...
          <button onClick={() => router.push('/admin/lupa-password')} className="text-blue-600 font-bold hover:underline">
            Kembali
          </button>
        </div>
      </div>
