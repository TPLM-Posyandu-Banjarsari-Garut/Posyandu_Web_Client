...
export default function PendaftaranBayi() {
  ...
  // Fetch current logged-in parent
  const { data: parentUser, isLoading: isParentLoading } = useOrangTuaCurrentUser();

  // Fetch all posyandus for dropdown selection
  const { data: posyandusData, isLoading: isPosyandusLoading } = useGetOrangTuaPosyandus({
    limit: 100,
  });

  // Create Child Mutation
  const createMutation = useCreateOrangTuaChild();
  ...
  const selectedPosyanduId = watch("posyandu_id");

  // Get detail of currently selected posyandu
  const selectedPosyandu = posyandusData?.data.find((p) => p.id === selectedPosyanduId);
  ...
  const onSubmit = handleSubmit((data) => {
    ...
    const payload: CreateChildPayload = {
      ...
      mother_name: parentUser?.name || null,
    };
    ...
    createMutation.mutate(payload, {
      onSuccess: () => {
        setShowSuccess(true);
        setTimeout(() => {
          router.push("/orangtua/data-bayi");
        }, 1500);
      },
      onError: (err: unknown) => {
  ...
  const isPending = isParentLoading || createMutation.isPending;
            ...
            <div className="bg-rose-50 border border-rose-100 text-rose-700 text-xs font-bold p-4 rounded-2xl flex items-center gap-3 mb-4 shadow-sm animate-shake">
              ...
              <div className="flex flex-col text-[11px] leading-relaxed">
                ...
                {Object.entries(errors).map(([field, err]) => (
                  <span key={field} className="block">• {err.message}</span>
                ))}
              </div>
            </div>
          )}
          ...
          <form onSubmit={onSubmit} className="flex flex-col gap-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      {...register("birth_weight", {
                        pattern: {
                          value: /^\d{1,3}(\.\d{1,2})?$/,
                          message: "Format BB Lahir tidak valid (contoh: 3.25)",
                        },
                      })}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      {...register("birth_length", {
                        pattern: {
                          value: /^\d{1,3}(\.\d{1,2})?$/,
                          message: "Format TB Lahir tidak valid (contoh: 50.5)",
                        },
                      })}
                      className="w-full px-3 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-sm transition-all font-semibold"
                      placeholder="cm"
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    <select
                      className={`w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 text-sm transition-all font-semibold pr-10 appearance-none ${
                        selectedPosyanduId ? "text-slate-800" : "text-slate-400"
                      }`}
                ...
                {/* Dynamic Posyandu details */}
                {selectedPosyandu && (
                  <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 text-xs font-semibold text-slate-600 space-y-2.5 animate-scale-up">
                    <div className="flex justify-between items-center">
                      <span>Desa / Kelurahan</span>
                      <span className="text-slate-800 font-bold">{selectedPosyandu.village_name || "-"}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>RT / RW</span>
                      <span className="text-slate-800 font-bold">
                        {selectedPosyandu.rt && selectedPosyandu.rw ? `${selectedPosyandu.rt} / ${selectedPosyandu.rw}` : "-"}
                      </span>
                    </div>
                    <div className="flex flex-col gap-1">
                      <span>Alamat Lengkap</span>
                      <span className="text-slate-800 font-bold text-[11px] leading-relaxed">{selectedPosyandu.address_line || "-"}</span>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="bg-white p-5 rounded-[1.5rem] shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-slate-100">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1.5 ml-1">
                    Nama Ibu (Terhubung Otomatis)
                  </label>
                  <input
                    type="text"
                    value={parentUser?.name || "Memuat..."}
                    className="w-full px-4 py-3 rounded-xl bg-slate-100 border border-slate-200 text-sm font-bold text-slate-500 pointer-events-none"
                    readOnly
                  />
                </div>
              </div>
            ...
            <div className="mt-4 pb-12">
              <button
                type="submit"
                disabled={isPending}
                className="w-full bg-blue-600 text-white rounded-[1.25rem] py-4 font-bold shadow-[0_8px_20px_rgba(37,99,235,0.25)] hover:bg-blue-700 active:scale-95 transition-all text-center flex justify-center items-center gap-2 disabled:opacity-75 disabled:cursor-not-allowed cursor-pointer"
              >
                {isPending ? (
                  <>
