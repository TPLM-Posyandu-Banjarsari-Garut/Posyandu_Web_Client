...
export default function PilihLayanan({ onSelect }: PilihLayananProps) {
  const [view, setView] = useState<'type' | 'select_child' | 'no_pregnancy_record'>('type');
  ...
  const { data: pregnancyData, isLoading: isLoadingPregnancy, refetch: refetchPregnancy } = useGetOrangTuaPregnancyRecords({
    is_active: true,
    limit: 10,
  });
  ...
  const handleSelectPregnancy = async () => {
    // If not loaded or empty, refetch just in case
    const currentRecords = pregnancyRecords;
    if (currentRecords.length > 0) {
      // Use the first active pregnancy record
      onSelect('pregnancy', null, currentRecords[0].id);
    } else {
  ...
  const handleSelectImunisasi = () => {
    if (childrenList.length === 1) {
      // Auto select if only 1 child exists
      onSelect('child_development', childrenList[0].id, null);
    } else {
    ...
    return (
      <div className="flex flex-col gap-4">
        <div>
          ...
          <p className="text-xs font-semibold text-slate-500">Pilih profil bayi untuk penjadwalan imunisasi/tumbuh kembang.</p>
        </div>
        ...
        <div className="flex flex-col gap-3">
          {isLoadingChildren ? (
            ...
            <div className="bg-white rounded-3xl p-6 border border-[#EBE8D8] text-center flex flex-col items-center gap-4">
              ...
              <p className="text-xs text-slate-500 font-semibold leading-relaxed max-w-[260px]">
                Anda harus mendaftarkan bayi Anda terlebih dahulu sebelum membuat janji imunisasi.
              </p>
              <Link
            ...
            childrenList.map((child) => (
              <button
                key={child.id}
                onClick={() => onSelect('child_development', child.id, null)}
                ...
                <div className="flex items-center gap-3.5 min-w-0 flex-1">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-slate-500 font-semibold mt-1">
                      NIK: {child.nik} · {child.gender === 'male' ? 'Laki-laki' : 'Perempuan'}
                    </p>
                  </div>
                </div>
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        ...
        <p className="text-xs font-semibold text-slate-500">Pilih jenis layanan konsultasi/pemeriksaan yang Anda butuhkan.</p>
      </div>
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-semibold mt-1">Konsultasi umum / keluhan kesehatan ringan</p>
            </div>
          </div>
          <svg className="w-4 h-4 text-slate-400 shrink-0" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
