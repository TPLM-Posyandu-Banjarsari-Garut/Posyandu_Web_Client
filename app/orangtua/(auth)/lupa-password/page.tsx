...
import { useForgetPasswordOrangTua } from "@/hooks/query/authOrangTua/useForgetPasswordOrangTua";

interface ForgetPasswordForm {
  email: string;
}

export default function LupaPasswordOrangTua() {
  const router = useRouter();
  const { register, handleSubmit, formState: { errors } } = useForm<ForgetPasswordForm>();
  const forgetPasswordMutation = useForgetPasswordOrangTua();

  const onSubmit = (data: ForgetPasswordForm) => {
    forgetPasswordMutation.mutate(data.email, {
      onSuccess: () => {
        router.push(`/orangtua/lupa-password/otp?email=${encodeURIComponent(data.email)}`);
      },
    });
  };
