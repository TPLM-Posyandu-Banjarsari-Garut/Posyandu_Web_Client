...
export default function OrangTuaAuth() {
  ...
  const handleSendOtp = () => {
    ...
    if (!regEmail.trim()) {
      setError("Email tidak boleh kosong untuk mengirim OTP");
      return;
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (activeTab === "login") {
      ...
      if (regPassword !== regConfirmPassword) {
        setError("Kata sandi verifikasi tidak cocok");
        return;
      }
      if (!otp.trim()) {
    ...
    <div className="min-h-screen bg-slate-100 font-sans pb-10 pt-4 px-0 sm:px-0 text-slate-800 flex justify-center">
      ...
      <div className="w-full max-w-md bg-white min-h-screen sm:min-h-[90vh] rounded-[2.5rem] relative shadow-2xl overflow-hidden flex flex-col border-[6px] border-white ring-1 ring-slate-200">
        ...
        <div className="bg-gradient-to-br from-blue-500 to-indigo-600 px-6 pt-10 pb-16 flex flex-col relative z-0 shrink-0">
          ...
          {/* Page Headers (Dynamic based on active tab) */}
        ...
        <div className="bg-white rounded-t-[2.5rem] -mt-8 pt-8 px-6 pb-8 flex-1 flex flex-col justify-between relative z-10">
                ...
                <>
                  ...
                  <div className="flex items-center gap-3.5 bg-slate-50 border border-slate-200/80 rounded-[1.25rem] p-3 hover:border-slate-350 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-500/10 transition-all">
                    ...
                    <button
                      ...
                      className="text-slate-400 hover:text-slate-650 transition-colors p-1 cursor-pointer"
                      title={regPasswordVisible ? "Sembunyikan" : "Tampilkan"}
                    >
                      {regPasswordVisible ? (
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2.2" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l18 18" />
                  ...
                  <div className="flex items-center gap-3.5 bg-slate-50 border border-slate-200/80 rounded-[1.25rem] p-3 hover:border-slate-350 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-500/10 transition-all">
                    ...
                    <div className="flex flex-col flex-1">
                      ...
                      <input
                        type={regConfirmPasswordVisible ? "text" : "password"}
                    ...
                    <button
                      type="button"
                      onClick={() => setRegConfirmPasswordVisible(!regConfirmPasswordVisible)}
                      className="text-slate-400 hover:text-slate-650 transition-colors p-1 cursor-pointer"
                      title={regConfirmPasswordVisible ? "Sembunyikan" : "Tampilkan"}
                    >
                  ...
                  {/* OTP Field with Kirim OTP Button layout */}
                  <div className="flex flex-col gap-1.5">
                    <div className="flex items-center gap-3.5 bg-slate-50 border border-slate-200/80 rounded-[1.25rem] p-3 hover:border-slate-350 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-500/10 transition-all">
                      ...
                      <button
                        ...
                        className="text-slate-400 hover:text-slate-650 transition-colors p-1 cursor-pointer"
                        title={otpVisible ? "Sembunyikan" : "Tampilkan"}
                      >
                        {otpVisible ? (
                          <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2.2" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l18 18" />
                    ...
                    {/* Kirim OTP Pill Button */}
                    <button
                      type="button"
                      onClick={handleSendOtp}
                      disabled={otpLoading || otpTimer > 0}
                      className="bg-blue-600 hover:bg-blue-700 text-white text-[10px] font-extrabold px-4 py-1.5 rounded-full w-fit mt-1 self-start active:scale-95 transition-all disabled:opacity-75 disabled:active:scale-100 flex items-center gap-1.5 shadow-[0_2px_8px_rgba(37,99,235,0.2)] cursor-pointer"
                    >
                      {otpLoading ? (
                        <>
          ...
          {/* Submit Action and Third-Party Login */}
          <div className="mt-8 flex flex-col gap-6">
            ...
            >
              {isLoading ? (
                ...
                // Keep label "Masuk" for both login and register to match mock-up literally,
                // but we also keep it incredibly flexible.
            ...
            >
              <svg className="w-5 h-5" viewBox="0 0 24 24" width="24" height="24">
                ...
                <path
                  fill="#34A853"
                  d="M12 24c3.24 0 5.95-1.08 7.93-2.91l-3.99-3.13a7.5 7.5 0 0 1-11.4 0H.55v3.13C2.53 21.08 7.53 24 12 24z"
                />
                <path
                  fill="#FBBC05"
                  d="M6.54 14.71a7.25 7.25 0 0 1 0-4.58V7h-4A11.94 11.94 0 0 0 12 12c0 1.2-.21 2.37-.58 3.48l3.99 3.13a7.25 7.25 0 0 1-8.87-3.9z"
                />
                <path
                  fill="#EA4335"
                  d="M12 4.75c1.77 0 3.35.61 4.6 1.8l3.42-3.42C17.93 1.19 15.22 0 12 0 7.53 0 2.53 2.92.55 7.15l3.99 3.13a7.25 7.25 0 0 1 7.46-5.53z"
                />
              </svg>
          ...
          <div className="bg-white w-full max-w-[290px] rounded-[2rem] p-6 shadow-2xl border border-slate-100 flex flex-col items-center justify-center animate-scale-up text-center">
            ...
            <p className="text-xs text-slate-500 leading-normal font-semibold">
              {successType === "login" 
                ...
                : "Akun Anda berhasil dibuat! Mengarahkan ke dasbor..."}
            </p>
          </div>
        </div>
      )}
